...
CacheTree::CacheTree(size_t nsamples, size_t nrules, double c, rule_t *rules,
                        rule_t *labels, rule_t *minority, int ablation, size_t max_depth,
                        bool calculate_size, char const *type)
    : root_(0), nsamples_(nsamples), nrules_(nrules), c_(c),
      num_nodes_(0), num_evaluated_(0), ablation_(ablation), max_depth_(max_depth), calculate_size_(calculate_size), min_objective_(0.5),
