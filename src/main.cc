...
int main(int argc, char *argv[]) {
    const char usage[] = "USAGE: %s [-b] "
        ...
        "-a (0|1|2) [-s] [-L latex_out] [-d max_depth]"
    ...
    while ((ch = getopt(argc, argv, "bsLc:p:v:n:d:r:f:a:u:")) != -1) {
        switch (ch) {
        case 'b':
            run_bfs = true;
            break;
    ...
    if (max_depth < 1) {
        error = true;
        _snprintf(error_txt, BUFSZ, "max_depth must be >= 1");
    }
    if (c < 0) {
        error = true;
    ...
    _snprintf(froot, BUFSZ, "../logs/for-%s-%s%s-%s-%s-removed=%s-max_num_nodes=%d-max_depth=%d-c=%.7f-v=%s-f=%d",
            pch ? pch + 1 : "",
            run_bfs ? "bfs" : "",
            ...
            max_num_nodes, max_depth, c, verbstr, freq);
    ...
    if(run_corels_begin(c, &verbstr[0], curiosity_policy, map_type, ablation, max_depth, calculate_size,
