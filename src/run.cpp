...
int run_corels_begin(double c, char* vstring, int curiosity_policy,
                  int map_type, int ablation, int max_depth, int calculate_size, int nrules, int nlabels,
                  ...
                  std::set<std::string>& verbosity)
{
    ...
    tree = new CacheTree(nsamples, nrules, c, rules, labels, meta, ablation, max_depth, calculate_size, type);
    if (verbosity.count("progress"))
        printf("%s", run_type);
